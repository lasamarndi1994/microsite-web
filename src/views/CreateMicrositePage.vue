                            ...
                            <v-row>
                                <v-col cols="12" md="12">
                                    <v-text-field v-model="title" label="Microsite Title"
                                        ...
                                        autocomplete="off" density="default" class="bg-white" hide-details="auto"
                                        :error-messages="titleError"></v-text-field>
                                </v-col>
                                <v-col cols="12" md="12">
                                    <v-text-field v-model="slug_url" label="Slug URL" placeholder="Your Slug URL"
                                        autocomplete="off" variant="outlined" density="default" class="bg-white"
                                        hide-details="auto" :error-messages="slugUrlError"></v-text-field>
                                </v-col>
                                <v-col cols="12" md="12">
                                    <v-text-field v-model="subTitle" label="Subtitle (Optional)" autocomplete="off"
    ...
    return new Promise((resolve) => {
        ...
        slugCheckTimeout = setTimeout(async () => {
            try {
                ...
                resolve(true); // Proceed anyway if API errors out
            }
        }, 500);
    });
})
...
watch(title, (newTitle) => {
    if (!isPreview.value) {
        slug_url.value = newTitle ? newTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') : '';
    }
})
